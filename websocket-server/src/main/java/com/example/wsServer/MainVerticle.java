...
import com.example.wsServer.Utils.KubernetesUtils;
...
public class MainVerticle extends VerticleBase {
  ...
  /**
   * Initializes Kubernetes/Istio resources if running in a Kubernetes environment.
   * Creates VirtualService and ServiceEntry for pod-level routing.
   */
  private void initializeKubernetesResources() {
    if (KubernetesUtils.isRunningInKubernetes()) {
      LOGGER.info("Kubernetes environment detected, initializing Istio resources...");
      try {
        kubernetesUtils = new KubernetesUtils();
        kubernetesUtils.createIstioResources();
        
        // Register shutdown hook to clean up resources on termination
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
          LOGGER.info("Shutdown hook triggered, destroying Istio resources...");
          if (kubernetesUtils != null) {
            kubernetesUtils.destroyIstioResources();
            kubernetesUtils.close();
          }
        }, "istio-cleanup-hook"));
        
        LOGGER.info("Istio resources created successfully");
      } catch (Exception e) {
        LOGGER.warning("Failed to initialize Kubernetes resources: " + e.getMessage());
        // Don't fail startup if Istio resource creation fails
      }
    } else {
      LOGGER.info("Not running in Kubernetes, skipping Istio resource initialization");
    }
  }

  @Override
  public Future<?> start() {
    // Initialize Kubernetes/Istio resources on startup
    initializeKubernetesResources();
    ...
    // Metrics endpoint for main-api placement service to query connection counts
    router.get("/metrics").respond(ctx -> {
      JsonObject metrics = roomManager.getMetrics();
      metrics.put("serverId", SERVER_ID);
      return Future.succeededFuture(metrics);
    });

    // WebSocket server only handles /websocket requests - no static content
    // Static content is served by main-api
  ...
  public Future<?> stop() throws Exception {
    LOGGER.info("Stopping MainVerticle, cleaning up Kubernetes resources...");
    
    // Clean up Istio resources when verticle stops
    if (kubernetesUtils != null) {
      try {
        kubernetesUtils.destroyIstioResources();
        kubernetesUtils.close();
        kubernetesUtils = null;
      } catch (Exception e) {
        LOGGER.warning("Error cleaning up Kubernetes resources: " + e.getMessage());
      }
    }
